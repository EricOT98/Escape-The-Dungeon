		...
		for (auto & layer : map_layers) {
			...
			for (auto y = 0; y < m_rows; ++y) {
				for (auto x = 0; x < m_cols; ++x) {
					...
					sf::Texture temp = m_tilesets.at(tset_gid);
					auto ts_width = temp.getSize().x;
					auto ts_height = temp.getSize().y;

					//Calculate the area on the tilesheet to draw
					auto region_x = (cur_gid % (ts_width / m_tileWidth))  * m_tileWidth;
					auto region_y = (cur_gid / (ts_width / m_tileHeight)) * m_tileHeight;

					//Calculate the world position of the tile
					auto x_pos = x * m_tileWidth;
					auto y_pos = y * m_tileHeight;

					//Finally actually adding the finished tile
					Tile t(m_tilesets.at(tset_gid), x_pos, y_pos,
