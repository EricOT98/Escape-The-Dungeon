...
public:    
    MapLayer(const tmx::Map& map, std::size_t idx)
    {
            ...
            idx < layers.size() && layers[idx]->getType() == tmx::Layer::Type::Tile)
        {
            ...
            m_chunkSize.x = std::floor(map.getBounds().width / tileSize.x) * tileSize.x;
            m_chunkSize.y = std::floor(map.getBounds().height / tileSize.y) * tileSize.y;
			...
			//m_chunkSize = sf::Vector2f(m_globalBounds.width, m_globalBounds.height);
